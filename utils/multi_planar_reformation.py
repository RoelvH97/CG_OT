...
class MPRTransform(nn.Module):
    ...
    def __init__(self, config, device="cuda", max_points=600):
        ...
        self.p_theta = config["p_theta"] / config["ps_polar"][1] * 2 * torch.pi
        self.max_points = max_points

        self.patch_size = config["ps"]
        self.spacing = config["spacing"]
        self.patch_size_polar = [config["ps_polar"][0], config["ps_polar"][1] + 2 * config["p_theta"]]
    ...
    def _generate_angles(self, n_samples: int, repeat: int = None) -> torch.Tensor:
        ...
        """
        angles = torch.linspace(0 - self.p_theta, 2 * torch.pi + self.p_theta, n_samples, device=self.device)
    ...
    @staticmethod
    def get_coordinate_frame_from_tu(tu: torch.Tensor,
                                     return_tangents: bool = False) -> Union[
        Tuple[torch.Tensor, torch.Tensor],
        Tuple[torch.Tensor, torch.Tensor, torch.Tensor]
    ]:
        """
        Get the local coordinate frame (t, u vectors) for the centerline
        """
        t, u = tu[:, :3], tu[:, 3:]
        t, u = t / torch.norm(t, dim=1, keepdim=True), u / torch.norm(u, dim=1, keepdim=True)

        v_vectors = torch.cross(u, t, dim=1)
        u_vectors = -u

        if return_tangents:
            return t, u_vectors, v_vectors
        else:
            return u_vectors, v_vectors
    ...
    def forward(self,
        ...
        Args:
            ...
            centerline: Tensor containing centerline (ordered)
        ...
        if mode == "default":
            ...
            angles = self._generate_angles(self.patch_size_polar[1], repeat=tangent_vectors.shape[0])
    ...
    def tube_vertices(self, n_points, radius=0.5):
        thetas = self._generate_angles(self.patch_size_polar[1])

        x_ring = radius * torch.cos(thetas)
        y_ring = radius * torch.sin(thetas)
        z_values = self.spacing_polar[2] * torch.arange(n_points, device=self.device)

        # create a grid of points in 3D space
        x_coords = x_ring.repeat(n_points).view(n_points, self.n_theta)
        y_coords = y_ring.repeat(n_points).view(n_points, self.n_theta)
        z_coords = z_values.repeat_interleave(self.n_theta).view(n_points, self.n_theta)

        vertices = torch.stack([x_coords.flatten(),
                            y_coords.flatten(),
                            z_coords.flatten()], dim=1).to(self.device)
        normals = torch.stack([x_coords.flatten(),
                            y_coords.flatten(),
                            torch.zeros_like(z_coords.flatten())], dim=1).to(self.device)
